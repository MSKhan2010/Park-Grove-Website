...
type GalleryCategoryId = 'warmth' | 'home' | 'community' | 'details' | 'sparkle';
type GallerySize = 'standard' | 'feature' | 'wide' | 'tall' | 'square';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const buildEncodedImagePath = (directory: string, fileName: string): string => {
        return `${normaliseDirectory(directory)}/${encodeFileName(fileName)}`;
    };

    const buildRawImagePath = (directory: string, fileName: string): string => {
        return `${normaliseDirectory(directory)}/${fileName}`;
    };

    const getFileNameCandidates = (number: number): string[] => {
    ...
    const buildImageAttempts = (image: GalleryImage): string[] => {
    ...
    const galleryGrid = document.querySelector<HTMLElement>('#galleryGrid');
    ...
    const galleryCountElements = document.querySelectorAll<HTMLElement>('[data-gallery-count]');
    const heroOpenButtons = document.querySelectorAll<HTMLElement>('[data-gallery-open]');
    ...
    const lightboxKicker = document.querySelector<HTMLElement>('#lightboxKicker');
    const lightboxTitle = document.querySelector<HTMLElement>('#lightboxTitle');
    const lightboxDescription = document.querySelector<HTMLElement>('#lightboxDescription');
    const lightboxCount = document.querySelector<HTMLElement>('#lightboxCount');
    const closeButtons = document.querySelectorAll<HTMLElement>('[data-lightbox-close]');
    const previousButton = document.querySelector<HTMLElement>('[data-lightbox-prev]');
    const nextButton = document.querySelector<HTMLElement>('[data-lightbox-next]');

    let activeFilter: GalleryCategoryId | 'all' = 'all';
    let visibleImages: GalleryImage[] = [...galleryImages];
    let currentLightboxIndex = 0;
    let lastFocusedElement: HTMLElement | null = null;

    const getCategoryById = (categoryId: GalleryCategoryId): GalleryCategory => {
        return categories.find((category) => category.id === categoryId) ?? categories[0];
    };

    const updateGalleryCounts = (): void => {
    ...
    const applyLetterLoop = (): void => {
        const headings = document.querySelectorAll<HTMLElement>('[data-gallery-letter-loop]');
    ...
    const setImageState = (host: HTMLElement | null, state: 'loading' | 'loaded' | 'missing'): void => {
        if (!host) return;
    ...
    const setManagedImageSource = (
        imageElement: HTMLImageElement,
        galleryImage: GalleryImage,
        options: {
            host?: HTMLElement | null;
            onMissing?: (attempts: string[]) => void;
            onLoaded?: (src: string) => void;
        } = {}
    ): void => {
        const host = options.host ?? imageElement.closest<HTMLElement>('.gallery-card, .hero-photo, .lightbox-image-wrap');
    ...
    const addCardTilt = (card: HTMLElement): void => {
    ...
    const openLightboxByNumber = (imageNumber: number): void => {
    ...
    const createGalleryCard = (image: GalleryImage, index: number): HTMLButtonElement => {
    ...
    const revealOnScroll = (): void => {
        const revealElements = document.querySelectorAll<HTMLElement>('.gallery-reveal, .gallery-card');
    ...
    const setActiveFilter = (filter: GalleryCategoryId | 'all'): void => {
        activeFilter = filter;
        ...
        document.querySelectorAll<HTMLElement>('.gallery-card').forEach((card) => {
            const category = card.dataset.category as GalleryCategoryId | undefined;
    ...
    const connectFilters = (): void => {
        filterButtons.forEach((button) => {
            const filter = button.dataset.galleryFilter as GalleryCategoryId | 'all' | undefined;
            if (!filter) return;
    ...
    const setLightboxImage = (image: GalleryImage): void => {
        ...
        const lightboxImageWrap = lightboxImage.closest<HTMLElement>('.lightbox-image-wrap');
    ...
    const showLightboxImage = (direction: 1 | -1): void => {
    ...
    const hydrateHeroImages = (): void => {
        heroOpenButtons.forEach((button) => {
            const imageNumber = Number(button.dataset.galleryOpen);
            if (!Number.isFinite(imageNumber)) return;

            const heroImage = button.querySelector<HTMLImageElement>('img');
            const heroLoader = button.querySelector<HTMLElement>('.hero-image-loader');
    ...
    const enhanceInternalScroll = (): void => {
        const exploreLink = document.querySelector<HTMLAnchorElement>('a[href="#galleryGrid"]');
